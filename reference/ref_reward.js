...
exports.load = async function (mod) {
  ...
  logger.info('load reward reference table ' + (val.err ? 'fail : ' + val.err : 'success !!'));
      ...
      val.comdb.query(query, [], function (err, results) {
        if (!err) {
          for (const info of results) {
            if (temp_data[info.code] == undefined)
              temp_data[info.code] = { reward_type: info.reward_type, total_rate: 0, list: [] };
